...
import { renderPage, getPageCount, generatePrintPdf as genPrintPdf } from './pdfRenderer.js';
...
export async function listPrintJobs(filters: Record<string, any>, userId: string, role: string) {
  const { bookshop_id, status, page: pageStr, limit: limitStr } = filters;
  ...
  return { printJobs: result.rows, total, page, limit, totalPages: Math.ceil(total / limit) };
}

export async function createPrintJob(data: { book_id: string; bookshop_id: string; copies?: number; notes?: string }, userId: string) {
  const { book_id, bookshop_id, copies, notes } = data;
  ...
  const result = await query(
    `INSERT INTO print_jobs (book_id, bookshop_id, requested_by, copies, notes, status)
     ...
     RETURNING *`,
    [book_id, bookshop_id, userId, copies || 1, notes],
  );

  return result.rows[0];
}

export async function updatePrintJobStatus(jobId: string, status: string) {
...
export async function listPrintSessions(filters: Record<string, any>, userId: string, role: string) {
  const { book_id, bookshop_id, page: pageStr, limit: limitStr } = filters;
  const page = Math.max(1, parseInt(pageStr as string, 10) || 1);
  const limit = Math.min(100, Math.max(1, parseInt(limitStr as string, 10) || 50));
  const offset = (page - 1) * limit;
  ...
  return { sessions: result.rows, total, page, limit, totalPages: Math.ceil(total / limit) };
}

export async function viewWatermarked(bookId: string, userId: string, role: string, queryParams: Record<string, any>) {
  const printToken = queryParams.token as string | undefined;
  ...
  if (printToken) {
    ...
    bookshopId = (queryParams.bookshop_id as string) || '';
    if (bookshopId) {
    ...
    const sessionResult = await query(
      ...
      [bookId, bookshopId || null, userId, previewToken],
    );
  ...
  return { totalPages, bookshopName, sessionId, copies };
}

export function getCachedSession(sessionId: string) {
  return pdfCache.get(sessionId) || null;
}

export async function getPage(bookId: string, pageNum: string, sessionId: string, ip?: string) {
  if (!sessionId) {
...
export async function countPrint(bookId: string, copies: number, userId: string, role: string, body: Record<string, any>, headers: Record<string, any>) {
  ...
  const book = await query('SELECT id, title FROM books WHERE id = $1', [bookId]);
  if (book.rows.length === 0) {
    throw new AppError(404, 'Book not found');
  }
  ...
  if (role === 'admin') {
    bookshopId = (body.bookshop_id as string) || null;
  } else {
  ...
  return { session: result.rows[0], print_token: printToken, token_expires_at: tenMinutesFromNow };
}

export async function generatePrintPdf(bookId: string, sessionId: string, copies: number, printToken: string, ip?: string) {
  if (!printToken) {
  ...
  return { pdf: resultPdf, title: book.rows[0].title || 'book' };
}
