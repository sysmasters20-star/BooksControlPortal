...
export default function AdminAuditLogs() {
  ...
  useEffect(() => {
    ...
    adminApi.getAuditLogs(params).then(({ data }) => {
      setLogs(data.auditLogs || []);
      setTotal(data.total || 0);
      setTotalPages(data.totalPages || 0);
    }).catch(() => {}).finally(() => setLoading(false));
  }, [filterAction, page, limit]);
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <select value={filterAction} onChange={(e) => { setFilterAction(e.target.value); setPage(1); }} className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 outline-none">
